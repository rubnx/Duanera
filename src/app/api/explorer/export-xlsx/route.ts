...
export async function GET(request: Request) {
  ...
  const productFacing = { productFacing: true }

  try {
    const { filters, plan, rows } = await listTradeRecordsForExport(
      db,
      searchParams,
      productFacing
    )
    ...
    const summary = await summarizeTradeRecords(db, filters, productFacing)
